...
/**
	构造地址, 
	@params action 应用模块的controller, action, do，格式为 'User/index'
	@params querystring 格式为 {参数名1 : 值1, 参数名2 : 值2}
*/
util.url = function (action, querystring) {
	var app = getApp();
	console.log('action', action)
	// appid 预留用于未来扩展一站点多开应用时区分各应用，目前无效固定为0
	var url = app.appinfo.approot + '/api/vppz/'+action+'?appid='+ app.appinfo.appid;
...
function getSign(url, data, token) {
	...
	if (sign || (data && data.sign)) {
		...
		token = token ? token : getApp().appinfo.token;
...
util.request = function (option) {
	...
	var ssid = getUrlParam(url, 'ssid');
	if (!ssid && !(option.data && option.data.ssid) && sessionid) {
	...
	if (option.cachetime) {
		//console.log('request to cachetime:'+url);
				...
				//} else {
					if (option.fail && typeof option.fail == 'function') {
						...
						if (response.data.msg) {
							...
							app.util.message(response.data.msg, redirect, 'error');
						}
					}
					return;
				//}
...
util.getVpUser = function (cb, code) {
...
util.checkSession = function(option) {
	...
	util.request({
		...
		showLoading: false,
		success: function (res) {
			//console.log("checkSession success1");
			//if (!session.data.errno) {
			if (res.data.code==1) {
		...
		fail: function() {
			//console.log("checkSession fail1");
			typeof option.fail == "function" && option.fail();
		}
	})
}
...
util.getUserInfo = function (cb, wxInfo) {
	var login = function() {
		...
		wx.login({
			success: function (res) {
        		...
        		console.log(wxInfo);
				util.getVpUser(function(userInfo) {
					console.log('getVpUser');
					...
					if (wxInfo) {
						...
						if (wx.canIUse('getUserProfile')) {
							...
							//		}else{typeof cb == "function" && cb(userInfo);}
							//	}
							//})
	...
	if (userInfo.sessionid) {
		...
		util.checkSession({
			...
			fail: function(){
				console.log('checkSession fail do');
				userInfo.sessionid = '';
				console.log('relogin');
 ...
 * navigate:/vp/pages/detail/detail 以 navigateTo 的方法跳转，
 * redirect:/vp/pages/detail/detail 以 redirectTo 的方式跳转，默认为 redirect
*/
