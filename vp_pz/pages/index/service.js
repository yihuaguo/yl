...
Page({
    ...
    onShareAppMessage() {
        let that = this;
        console.log('share:',that.data);
        let Str = JSON.stringify(that.data);
        Str = Str.replace(/%/g, '%25');
        let obj = encodeURIComponent(Str);
        return {
            title: "填写服务订单",//自定义标题, 默认值小程序名称（当前小程序名称）
            path: 'vp_pz/pages/index/service?obj=' + obj,
    ...
    serviceSave: function (e) {
        ...
        // 联系电话验证
        if (!e.detail.value.tel&&!self.data.phone) {
            return wx.showToast({
                title: '请填写您的联系电话',
                icon: 'none',
                duration: 2000
        ...
        order.tel = e.detail.value.tel || self.data.phone;
        ...
        order.coupon_id = self.data.coupon_id;
        // 序列化提交数据对象
